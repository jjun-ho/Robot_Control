    ...
    %Simulation
        ...
        %Set Target Position  
            ...
            elseif(time < 2.0)
                ...
                %Get Dynamics
                J = get_Jacobian3(q(1),q(2),q(3));        %jacobian 
                %dJ = get_dJacobian3(q(1),q(2),q(3));     %jacobian dot
                dJ = (J - pre_J)/delta_t;
                pre_J = J; 

                X = get_Kinematics3(q(1),q(2),q(3));      % position
                %dX = get_dKinematics3(q(1),q(2),q(3));   % position dot
                dX = J*dq;                                % end effector velocity

                %dX = (X-pre_X)/delta_t;
                %pre_X = X;

                D = get_Inertia3(q(2),q(3));                  %inertia
                H = get_Coriollis3(q(2),q(3), dq(1),dq(2),dq(3));         %coriollis
                C = get_Gravity3(q(1),q(2),q(3));             % gravity
